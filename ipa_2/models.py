...
# General classes for both experiments:
...
class Experiment(models.Model):
    ...
    ps_l_A = models.IntegerField(default=30)
    ps_h_A = models.IntegerField(default=30)
    ps_l_C = models.IntegerField(default=30)
    ps_h_C = models.IntegerField(default=30)
...
class Subject(ProfileModel):
    ...
    context_group = models.ForeignKey(Context,  null=True, on_delete=models.SET_NULL) # i.e. romance, conflict, friendship and trade
...
class UserToSubjectIPA2(models.Model):
...
def create_user_subject(sender, instance, created, **kwargs):
    if created:
        if instance.first_name=="IPA_2":
            UserToSubjectIPA2.objects.create(user=instance)

@receiver(post_save, sender=User)
def save_user_subject(sender, instance, **kwargs):
    try:
        instance.usertosubjectipa2.save()
    except:
        pass
