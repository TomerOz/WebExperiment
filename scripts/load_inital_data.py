...
def run():
    ...
    def create_feature_labels(features_df): # TODO seperate code-english name from presenting name
        for index, row in features_df.iterrows():
            feature_query = FeatureLabels.objects.filter(feature_name=row.feature)
            if len(feature_query) == 0:
                new_feature = FeatureLabels(feature_name=row.feature, right_end=row.right_end, left_end=row.left_end)
                new_feature.save()
